<template>
  <a-modal
    ...
    :mask-closable="false"
    :open="open"
    :title="$t('config.dashboardGlobalOverview')"
    :footer="null"
    width="95vw"
  ...
  >
    <div class="py-2">
      <div class="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <a-select
          v-model:value="globalQuickRange"
          class="w-32"
        ...
        <a-divider type="vertical" class="mx-1!" />
        <a-radio-group v-model:value="globalGranularity" @change="refreshChart">
        ...
        <a-divider type="vertical" class="mx-1!" />
        <a-select
          v-model:value="globalSelectedModelId"
          class="w-44"
          @change="refreshChart"
        >
          <a-select-option value="all">{{
            $t('config.dashboardAllModels')
          }}</a-select-option>
          <a-select-option v-for="m in modelStats" :key="m.id" :value="m.id">{{
            m.name
          }}</a-select-option>
        </a-select>
        <div class="ml-auto flex items-center gap-5 text-sm">
          <div class="text-center">
            <div class="text-xs text-gray-400">
              {{ $t('config.dashboardTotalCallsShort') }}
            </div>
            <div class="font-bold text-gray-800">
              {{ globalTotals.calls.toLocaleString() }}
            </div>
          </div>
          <div class="text-center">
            <div class="text-xs text-gray-400">
            ...
            <div class="font-bold text-primary-600">
              {{ formatTokens(globalTotals.inputTokens) }}
            </div>
          </div>
          <div class="text-center">
            <div class="text-xs text-gray-400">
            ...
            <div class="font-bold text-green-600">
              {{ formatTokens(globalTotals.outputTokens) }}
            </div>
          </div>
        </div>
      </div>
      <div class="h-80 w-full bg-gray-50 rounded-lg p-2">
...
const globalDateRange = ref<[Dayjs, Dayjs] | null>(null)
const globalQuickRange = ref('7d')
const globalGranularity = ref<'day' | 'hour'>('day')
const globalSelectedModelId = ref('all')
...
watch(
  ...
  async (val) => {
    if (val) {
      ...
      globalQuickRange.value = '7d'
      globalSelectedModelId.value = 'all'
      globalGranularity.value = 'day'
      await refreshChart()
    }
  }
)
...
const globalTotals = computed(() => {
  const list =
    globalSelectedModelId.value === 'all'
      ? props.modelStats
      : props.modelStats.filter((m) => m.id === globalSelectedModelId.value)
  return {
    calls: list.reduce((s, m) => s + m.calls, 0),
    inputTokens: list.reduce((s, m) => s + m.inputTokens, 0),
    outputTokens: list.reduce((s, m) => s + m.outputTokens, 0),
  }
})
...
const onQuickRangeChange = (val: string) => {
  ...
  globalDateRange.value = [start, dayjs()]
  refreshChart()
}

const refreshChart = async () => {
  const start = globalDateRange.value?.[0] || dayjs().startOf('day')
  const end = globalDateRange.value?.[1] || dayjs()
  const selected =
    globalSelectedModelId.value === 'all'
      ? null
      : props.modelStats.find((m) => m.id === globalSelectedModelId.value)
  const params = {
  ...
  if (globalGranularity.value === 'hour') {
