...
<template>
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 w-[220px]">
    ...
    <div class="p-2 text-xs text-gray-500 space-y-1">
      <div>
        {{ t('workflowEditor.fileSource') }}:
        {{
          properties?.inputFields?.find((f: any) => f.name === 'Src')?.value ||
          t('workflowEditor.notSet')
        }}
      </div>
      <div>
        {{ t('workflowEditor.fileTarget') }}:
        {{
          properties?.inputFields?.find((f: any) => f.name === 'Dest')?.value ||
          t('workflowEditor.notSet')
        }}
      </div>
    </div>
