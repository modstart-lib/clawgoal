...
const effectiveHeight = computed(() =>
  props.autoHeight ? 'auto' : props.height
)
...
const getThemeExtension = () => {
  ...
  const rootHeight = props.autoHeight ? 'auto' : props.height
  // autoHeight 模式：minHeight 设在 .cm-content/.cm-gutter 上（CM6 推荐），让编辑器随内容自动伸展
  // 3行 × 20px/行 = 60px（13px 字体，约 1.5 行高）
...
<template>
  <div
    ...
    :class="effectiveTheme === 'dark' ? 'theme-dark' : 'theme-light'"
  >
...
.code-editor-wrapper.theme-light:focus-within {
  ...
  box-shadow: 0 0 0 3px
    color-mix(in srgb, var(--token-primary) 15%, transparent);
}
...
.code-editor-wrapper.theme-dark:focus-within {
  border-color: var(--token-primary);
  box-shadow: 0 0 0 3px
    color-mix(in srgb, var(--token-primary) 20%, transparent);
}
